...
new _promise2.default(function (resolve, reject) {
    ...
    return new _promise2.default(function (resolve, reject) {
        VK.api('friends.get', { fields: 'photo_50', v: '5.69' }, function (data) {
            if (data.error) {
                reject(data.error);
            } else {
    ...
    var listOfSelected = document.querySelector('.container-drop');
    var listOfAll = document.querySelector('.friends__container');
    var saveBtn = document.querySelector('.save-btn');

    if (localStorage.allFriends) {
        var allFriends = JSON.parse(localStorage.allFriends),
            selectedFriends = JSON.parse(localStorage.selectedFriends);

        for (var key in allFriends) {
            allFriendsList[key] = allFriends[key];
        }
        for (var _key in selectedFriends) {
            selectedFriendsList[_key] = selectedFriends[_key];
        }
        renderTemp(allFriendsList, listOfAll);
        renderTemp(selectedFriends, listOfSelected, 'friend__add_rotate');
    } else {
        for (var _key2 in friends) {
            allFriendsList[_key2] = friends[_key2];
        }
        selectedFriendsList = {
            items: []
        };
    ...
    function saveLists(e) {
        ...
        localStorage.allFriends = (0, _stringify2.default)(allFriendsList);
        localStorage.selectedFriends = (0, _stringify2.default)(selectedFriendsList);
    ...
    function keyUpAnother(e) {
        var nameSelected = document.querySelectorAll('.container-drop .friend__name');
        var nameAll = document.querySelectorAll('.friends__container .friend__name');
        ...
        if (e.target.classList.contains('search-list')) {
            ...
            for (var _i = 0; _i < nameAll.length; _i++) {
                if (isMatching(nameAll[_i].innerText, e.target.value)) {
                    nameAll[_i].parentNode.style.display = 'block';
                } else if (!isMatching(nameAll[_i].innerText, e.target.value)) {
                    nameAll[_i].parentNode.style.display = 'none';
                }
                if (!e.target.value) {
                    nameAll[_i].parentNode.style.display = 'block';
                }
    ...
    document.onmousemove = function (e) {
        ...
        if (!dragObj.avatar) {
            ...
            var coords = getCoords(dragObj.avatar);
    ...
    function selectedUpd(elem) {
        allFriendsList.items.forEach(function (item, i) {
    ...
    function allUpd(elem) {
        selectedFriendsList.items.forEach(function (item, i) {
            if (+elem.dataset.id === item.id) {
    ...
    function dropingElem(elem) {
        ...
        if (elem.classList.contains('container-drop')) {
            dragObj.elem.querySelector('.friend__add').classList.add('friend__add_rotate');
            selectedUpd(dragObj.elem);
        } else {
            dragObj.elem.querySelector('.friend__add').classList.remove('friend__add_rotate');
            allUpd(dragObj.elem);
        }
    }
